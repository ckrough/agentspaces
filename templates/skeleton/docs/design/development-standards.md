...
Standards for {{ project_name }} development.

## Git Workflow: {{ git_workflow | default("GitHub Flow") }}
...
### Commit Messages ({{ commit_convention | default("Conventional Commits") }})
...
# Tests with coverage
pytest --cov=src --cov-report=term-missing --cov-fail-under={{ test_coverage_target | default(80) }}
...
ruff format src/ tests/ && \
mypy src/ --strict && \
pytest --cov=src --cov-fail-under={{ test_coverage_target | default(80) }}
```
...
## Docstrings ({{ docstring_style | default("Google") }} Style)
...
{{ test_coverage_target | default(80) }}% minimum for new code.
...
- [ ] New code has tests ({{ test_coverage_target | default(80) }}% coverage)
...
{% if pre_approved_deps %}
### Pre-Approved Libraries

| Category | Libraries |
|----------|-----------|
{% for category, libs in pre_approved_deps.items() %}
| {{ category }} | {{ libs | join(", ") }} |
{% endfor %}
{% else %}
### Pre-Approved Libraries
